本文翻译自docker官网：[https://docs.docker.com/develop/develop-images/dockerfile_best-practices/](https://docs.docker.com/develop/develop-images/dockerfile_best-practices/)
...
have." For example, you don鈥檛 need to include a text editor in a database image.
...
Here鈥檚 an example from the [`buildpack-deps` image](https://github.com/docker-library/buildpack-deps):
...
what鈥檚 in the cache. This technique can also reduce failures due to unanticipated changes
...
of `CMD ["executable", "param1", "param2"鈥`. Thus, if the image is for a
...
`docker run -it python`, you鈥檒l get dropped into a usable shell, ready to go.
...
variables specific to services you wish to containerize, such as Postgres鈥檚
`PGDATA`.
...
RUN curl -SL https://example.com/postgres-$PG_VERSION.tar.xz | tar -xJC /usr/src/postgres && 鈥�
...
is preferred. That鈥檚 because it鈥檚 more transparent than `ADD`. `COPY` only
...
For other items (files, directories) that do not require `ADD`鈥檚 tar
...
> the "next" UID/GID is assigned regardless of image rebuilds. So, if it鈥檚
...
running it as non-`root`, consider using [鈥済osu鈥漖(https://github.com/tianon/gosu).
...
like `RUN cd 鈥� && do-something`, which are hard to read, troubleshoot, and
maintain.
...
`Dockerfile`, as you can see in [Ruby鈥檚 `ONBUILD` variants](https://github.com/docker-library/ruby/blob/c43fef8a60cea31eb9e7d960a076d633cb62ba8d/2.4/jessie/onbuild/Dockerfile).
